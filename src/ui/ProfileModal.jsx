...
function Field({
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      />
      {error && (
        <div style={{ fontSize: 11.5, color: "#c06878", marginTop: 4 }}>
          {error}
        </div>
      )}
      {success && (
        <div style={{ fontSize: 11.5, color: "#6ed8a0", marginTop: 4 }}>
          {success}
        </div>
      )}
    </div>
        ...
        color: isDanger
          ...
          : isGhost
            ...
            : disabled || busy
              ...
              : "#cbbff5",
        boxShadow:
          h && !disabled && !busy && !isDanger && !isGhost
            ? "0 4px 16px rgba(100,40,220,0.2)"
            : "none",
      }}
    >
      {busy ? "Saving…" : label}
  ...
  return (
    <div
      style={{
        ...
        border: "1px solid rgba(112,76,196,0.3)",
        borderRadius: 10,
        padding: "14px 16px",
        flex: 1,
        minWidth: 0,
...
export function ProfileModal({ user, onUserUpdate, onLogout, onClose }) {
  ...
  useEffect(() => {
    museumService.getMyMuseums(user.id).then((museums) => {
      ...
      const totalRooms = museums.reduce(
        (acc, m) => acc + (m.roomCount ?? 0),
        0,
      );
      setStats({
  ...
  return (
    ...
    >
      <style>{`
        .profile-modal button:focusPassword-visible, .profile-modal input:focus-visible {
      ...
      <div
        ...
        style={{
          ...
          margin: "0 16px",
          background: "#1a1a22",
          border: "1px solid rgba(112,76,196,0.5)",
          borderRadius: 16,
          padding: "28px 32px",
          boxShadow:
            "0 0 0 1px rgba(255,255,255,0.05) inset, 0 32px 80px rgba(0,0,0,0.6)",
        }}
      >
        {/* Header */}
        <div
          ...
          {stats ? (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <StatCard
                label="Rooms"
                value={stats.rooms}
                sub="across all museums"
              />
              <StatCard
                label="Visits"
                value={stats.visits}
                sub="on your public museums"
              />
            </div>
          ) : (
        ...
        <Section title="Danger zone">
          {!showDelete ? (
            ...
            >
              <div style={{ fontSize: 13, color: T.textMuted }}>
                Permanently delete your account and all museums.
              </div>
              <Btn
