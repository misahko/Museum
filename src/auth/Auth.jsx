  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{
        ...
        color: 'rgba(185,185,198,0.75)', marginBottom: 7,
      }}>{label}</label>
      <input
        ...
        style={{
          ...
          borderRadius: 9, color: T.text, fontSize: 13.5, outline: 'none',
          fontFamily: 'inherit',
      ...
      />
      {error && (
        <div style={{ fontSize: 11.5, color: '#c06878', marginTop: 5 }}>{error}</div>
      )}
    </div>
  );
}

function SubmitBtn({ label, disabled, hover, onMouseEnter, onMouseLeave, onFocus, onBlur }) {
  return (
    <button
      ...
      style={{
        ...
        border: disabled ? '1px solid rgba(255,255,255,0.05)' : '1px solid rgba(128,88,210,0.5)',
    ...
    <p style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: T.textMuted }}>
      ...
      <button
        ...
        style={{
          ...
          color: h ? '#cbbff5' : T.accent, fontWeight: 600, fontSize: 13,
          textDecoration: h ? 'underline' : 'none', padding: 0,
...
export function Auth({ onAuth, onClose }) {
  ...
  function validate() {
    ...
    if (mode === 'register' && pass !== pass2)     e.pass2 = 'Passwords do not match.';
    return e;
  }
  ...
  async function handleSubmit(e) {
    ...
    setErrors({}); setApiErr(''); setBusy(true);
    try {
  ...
  return (
    ...
    >
      ...
      <div className="auth-root" style={{
        width: '100%', maxWidth: 420, padding: '0 20px', boxSizing: 'border-box', position: 'relative',
      }}>
        {onClose && (
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{
            ...
            background: 'linear-gradient(135deg, rgba(128,88,210,0.35) 0%, rgba(90,55,175,0.2) 100%)',
          ...
          <div style={{
            ...
            textTransform: 'uppercase', color: 'rgba(190,188,210,0.6)',
          }}>Research Museum</div>
        </div>
        ...
        <div style={{
          ...
          boxShadow: '0 0 0 1px rgba(255,255,255,0.05) inset, 0 24px 60px rgba(0,0,0,0.55)',
        }}>
          <h2 style={{
            margin: '0 0 6px', fontSize: 20, fontWeight: 700,
            letterSpacing: '-0.3px', color: '#dcdce6',
          }}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            {!isLogin && (
              <Field label="Name" value={name} onChange={e => setName(e.target.value)}
                placeholder="Your name" error={errors.name} disabled={busy} />
            )}
            <Field label="Email" type="email" value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="you@example.com" error={errors.email} disabled={busy} />
            <Field label="Password" type="password" value={pass}
              onChange={e => setPass(e.target.value)}
              placeholder={isLogin ? 'Password' : 'At least 6 characters'}
              error={errors.pass} disabled={busy} />
            {!isLogin && (
              <Field label="Confirm password" type="password" value={pass2}
                onChange={e => setPass2(e.target.value)}
                placeholder="Repeat password" error={errors.pass2} disabled={busy} />
            )}
            ...
            {apiErr && (
              <div style={{
                ...
                color: '#c06878', fontSize: 13,
              }}>{apiErr}</div>
            )}

            <SubmitBtn
              label={busy ? (isLogin ? 'Signing in…' : 'Creating…') : (isLogin ? 'Sign in' : 'Create account')}
              disabled={busy}
              hover={btnH} onMouseEnter={() => setBtnH(true)} onMouseLeave={() => setBtnH(false)}
              onFocus={() => setBtnF(true)} onBlur={() => setBtnF(false)}
            />
          </form>
